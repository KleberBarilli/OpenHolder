...
Open-source portfolio dashboard built with **Phoenix 1.8**, **LiveView**, **SQLite**, and **Tailwind v4**.
Tracks asset allocation across multiple classes (stocks BR, FIIs, fixed income, US stocks, REITs, ETFs, crypto), scores assets with fundamental criteria, and suggests rebalancing.
...
No external database server needed — SQLite files are created automatically (`holder_dev.db`, `holder_test.db`).

## Project guidelines

- Run `mix precommit` when you are done with all changes and fix any pending issues (compiles with `--warnings-as-errors`, unlocks unused deps, formats, and runs tests)
- Use the already included `:req` (`Req`) library for HTTP requests — **avoid** `:httpoison`, `:tesla`, and `:httpc`
- Default locale is `pt_BR`, also supports `en` — use `Gettext` for all user-facing strings
...
| `PHX_HOST` | no | Hostname (defaults to `example.com`) |
...
- The `HolderWeb.Layouts` module is aliased in `holder_web.ex`, so you can use it without needing to alias it again
...
#### Custom CSS classes (defined in `assets/css/app.css`)

The project defines a dark theme with custom component classes. **Prefer these over ad-hoc styles** when they fit:

| Class | Purpose |
|---|---|
| `.glass-card` | Dark card with border and blur backdrop |
| `.glass-card-hover` | Same as above with emerald hover glow |
| `.badge-buy` | Green badge for "buy" signals |
| `.badge-hold` | Gray badge for "hold" signals |
| `.input-field` | Styled dark input with emerald focus ring |
| `.btn-primary` | Emerald solid button |
| `.btn-ghost` | Transparent button with hover fill |
| `.tab-active` / `.tab-inactive` | Tab navigation states |
| `.glow-line` | Decorative gradient divider |

Custom theme tokens are defined via `@theme` — use them via Tailwind utilities (e.g. `text-text-primary`, `bg-surface-1`, `text-accent-emerald`).
...
- Elixir's builtin OTP primitives like `DynamicSupervisor` and `Registry` require names in the child spec, such as `{DynamicSupervisor, name: Holder.MyDynamicSup}`, then you can use `DynamicSupervisor.start_child(Holder.MyDynamicSup, child_spec)`
...
- You **never** need to create your own `alias` for route definitions! The `scope` provides the alias:
      ...
      scope "/admin", HolderWeb.Admin do
        pipe_through :browser

        live "/users", UserLive, :index
      end

  the UserLive route would point to the `HolderWeb.Admin.UserLive` module
...
- For "app wide" template imports, you can import/alias into `holder_web.ex`'s `html_helpers` block, so they will be available to all LiveViews, LiveComponents, and all modules that do `use HolderWeb, :html`
...
- LiveViews should be named like `HolderWeb.WeatherLive`, with a `Live` suffix. When you go to add LiveView routes to the router, the default `:browser` scope is **already aliased** with the `HolderWeb` module, so you can just do `live "/weather", WeatherLive`
...
Where the server handles it via:
...
When using changesets, the underlying data, form params, and errors are retrieved from it. The `:as` option is automatically computed too. E.g. if you have a user schema:
    ...
    defmodule Holder.Users.User do
      use Ecto.Schema
      ...
    end
